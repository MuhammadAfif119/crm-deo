...
const ProjectCard = ({
  projectData,
  handleOpenModalProject,
  handleOpenModaProjectTeam,
}) => {
      ...
      {projectData?.length > 0 && (
        <SimpleGrid columns={[1, 2, 3]} gap={3}>
          {projectData.map((x, index) => {
            const isActive = topicsList?.includes(
              `${globalState.currentCompany}-${x.id}`
            );
            ...
            return (
              ...
              >
                <HStack>
                  ...
                  <Spacer />
                  <Box
                    bgColor={isActive ? "green.100" : "white"}
                    onClick={
                      isActive
                        ? () => handleNotif(x.id, false)
                        : () => handleNotif(x.id, true)
                    }
                    p={1}
                    borderRadius="full"
                    borderWidth={1}
                ...
                <AvatarGroup size="sm" gap="1" max={4}>
                  {x?.managers?.length > 0 &&
                    x?.managers.map((y, i) => {
                      const user = x?.usersProjectData?.find(
                ...
                <AvatarGroup size="sm" gap="1" max={4}>
                  {x?.usersProjectData?.length > 0 &&
                    x?.usersProjectData.map((y, i) => (
                      <Avatar key={i} name={y?.name} />
                    ))}
                </AvatarGroup>
                <Button
