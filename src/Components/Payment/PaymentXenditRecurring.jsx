import { Button, Divider, Flex, Heading, HStack, Input, Spacer, Stack, Text, useToast } from '@chakra-ui/react'
import axios from 'axios';
import React, { useEffect, useState } from 'react'
import _axios from '../../Api/AxiosBarrier';
import { addDocumentFirebase, updateDocumentFirebase } from '../../Api/firebaseApi';
import { formatFrice } from '../../Utils/Helper';

function PaymentXenditRecurring({ dataLeads, packageActive, dataForm, membershipList }) {
  ...
  const membershipData = membershipList.find((x) => x.package_code === packageActive)
  ...
  const handlePaymentTransfer = async (id, updatedOrder, fixPrice) => {
    try {
      ...
      if (res.status === true) {
        ...
        try {
          const result = await updateDocumentFirebase(collectionName, docName, dataUpdetOrder);
        ...
        toast({
          title: 'Warning!',
          description: 'Terjadi Kesalahan pembayaran, Silahkan menghubungi Admin.',
          status: "warning",
          duration: 9000,
          isClosable: true,
        });
  ...
  const handleOrderPayConfirm = async (priceFix) => {
    ...
    addDocumentFirebase("orders", updatedOrder, dataForm.companyId).then((x) => {
          ...
          <Stack w="full" p="4" borderColor="gray.300" borderWidth={1} rounded="md" spacing={2}>
            ...
            <Flex direction="column">
              ...
              <HStack justifyContent="space-between" fontSize="sm" my={1}>
                ...
                <Spacer />
                <Text textAlign="right" textTransform={'capitalize'}>{packageActive}</Text>
              </HStack>
      ...
      <Stack>
        ...
        <Stack>
          <HStack>
            <Stack>
              <Text>Amount :</Text>
              <Text fontWeight={500} >Rp. {formatFrice(Number(membershipData?.package_amount))}</Text>
            </Stack>
            <Spacer />

            <Stack>
              <Text>Duration :</Text>
              <Text fontWeight={500} > {membershipData?.package_expired_duration || membershipData?.package_expired}</Text>
            </Stack>
          </HStack>
          ...
          <Button colorScheme="green"
            ...
            onClick={() => handleOrderPayConfirm(membershipData.package_amount)}>Bayar</Button>
