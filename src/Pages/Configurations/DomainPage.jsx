...
function DomainsPage() {
  ...
  const getData = () => {
    ...
    getCollectionFirebase("domain_lists", conditions).then((x) => {
      console.log(x);
      if (x) {
        setData(x);
      }
  ...
  console.log(data, "ini data domain");
  ...
  const deleteDomain = async (i) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const deleteDomainResult = await deleteDomainCustom(
          data[i].domain,
          data[i].projectVercel
        );
        if (deleteDomainResult.status) {
          ...
          domain.domain = domain.domain.filter(function (e) {
            return e !== data[i].domain;
          });
          await setDocumentFirebase("domains", data[i].projectId, domain);
          getData();
          Swal.fire("Deleted!", "Success to delete domain", "success");
        } else {
          Swal.fire({
            icon: "error",
            title: "Oops...",
  ...
  const checkDomain = async (i) => {
    const domainResult = await checkDomainCustom(
      data[i].domain,
      data[i].projectVercel
    );
  ...
  useEffect(() => {
    ...
    if (globalState?.currentCompany) getData();
                    ...
                    <Td>
                      <HStack textAlign="center">
                        {x.server === "netlify" ? (
                          <Image
                            w="50px"
                            src="https://upload.wikimedia.org/wikipedia/commons/thumb/9/97/Netlify_logo_%282%29.svg/600px-Netlify_logo_%282%29.svg.png"
                          />
                        ) : (
                          <svg
