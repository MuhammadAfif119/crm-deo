    ...
    return (<>
        <Box>
            <HStack>
                ...
                <Spacer/>
                <Button colorScheme='green' onClick={()=>navigate("new")}>Add</Button>
            </HStack>
            <SimpleGrid columns={{base:3,lg:4}} p='2' gap='1'>
                {data?data.map((x,i)=>
                    <Box key={i} p='5' shadow='base' minH='100px'>
                    <Link to={`${x.id}`}>
                        ...
                        <Flex overflowY='auto'>
                        {x.modules?.map((z)=><Badge colorScheme='yellow'm='0.5'>{z}</Badge>)}
                        </Flex>
