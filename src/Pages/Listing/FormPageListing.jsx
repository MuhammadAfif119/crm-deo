...
import { addDocumentFirebase, arrayUnionFirebase, setDocumentFirebase, uploadFile } from '../../Api/firebaseApi';
...
function FormPageListing() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (docID) {
                ...
                try {
                    await arrayUnionFirebase(categoryCollectionName, docName, categoryField, categoryValues);
                } catch (error) {
                    // Jika dokumen belum ada, tambahkan dokumen baru dengan nilai awal
                    if (error.message.includes('No document to update')) {
                        await setDocumentFirebase(categoryCollectionName, docName, { [categoryField]: categoryValues }, companyId);
                    } else {
                ...
                for (const module of modules) {
                    ...
                    try {
                        await arrayUnionFirebase(subCollectionName, subDocName, subField, subValues);
                    } catch (error) {
                        // Jika dokumen belum ada, tambahkan dokumen baru dengan nilai awal
                        if (error.message.includes('No document to update')) {
                            await setDocumentFirebase(subCollectionName, subDocName, { [subField]: subValues }, companyId);
                        } else {
                            console.log('Terjadi kesalahan:', error);
                        }
                    }
    ...
    const handleCategoryChange = (e) => {
      ...
      const handleCategoryDelete = (categories) => {
        setCategory((prevCategories) => prevCategories.filter((cat) => cat !== categories));
      };
    ...
    return (
        <Stack>
            <Grid templateColumns={{ base: '1fr', md: '1fr 2fr' }} gap={5} p={3} mt={5}>
                <VStack spacing={4} align="start">
                    ...
                    <FormControl id="category">
                        ...
                        <HStack spacing={2}>
                            ...
                            <Button colorScheme="teal" onClick={handleCategoryAdd}>Tambah</Button>
                        </HStack>
                        <Stack direction="row" spacing={2} mt={2}>
                            {category.map((category) => (
                                <Box key={category} display="flex" alignItems="center" bg="teal.100" borderRadius="md" px={2} py={1}>
                        ...
                        <HStack key={index}>
                            <FormControl id={`detail-key-${index}`}>
                                ...
                                <Input
                                    type="text"
                                    value={detail.key}
                                    onChange={(e) => handleDetailChange(index, e.target.value, detail.value)}
                                />
                            </FormControl>
                            <FormControl id={`detail-value-${index}`}>
                                ...
                                <Input
                                    type="text"
                                    value={detail.value}
                                    onChange={(e) => handleDetailChange(index, detail.key, e.target.value)}
                                />
                            </FormControl>
                            <IconButton
                                icon={<MdDelete />}
                    ...
                    <FormControl id="modules">
                        ...
                        <Checkbox value="rms" onChange={handleModulesChange}>RMS</Checkbox>
                        <Checkbox value="listing" onChange={handleModulesChange}>Listing</Checkbox>
                        <Checkbox value="lms" onChange={handleModulesChange}>LMS</Checkbox>
                    </FormControl>
