...
function EditLesson() {
  ...
  const handleDelete = async (type) => {
    if (type === "image") {
      ...
      deleteFileFirebase(uid, "lessons", secondSplit[2]).then(() => {
        updateDoc(
          doc(db, `courses/${params.id_course}/lessons`, `${params.id_lesson}`),
          {
            thumbnail: [],
          }
        ).then(() => {});
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          let newFind = course?.lessons?.find(
            (item) =>
              item?.id === params.id_lesson || item?.title === lesson?.title
          );
          const splitArr =
            lesson?.thumbnail && lesson?.thumbnail[0]?.split("?");
  ...
  // console.log(lesson.status, "ini lessons");
    ...
    <Stack spacing={5}>
      ...
      <Stack>
        ...
        <Text color="red" fontWeight="bold" fontSize={12}>
          '.avi', '.mp4', '.mpeg', '.ogv', '.webm', '.3gp', '.mov', '.mkv',
          '.audio'
        </Text>
      </Stack>
      <Tabs
    ...
    <>
      <Box>
        <Stack>
          ...
          <HStack>
            <Heading>{lesson?.title ? lesson.title : <></>}</Heading>
            <Spacer />
            <HStack>
        ...
        <Flex mt={10} gap={4}>
          <Box width="70%" bg="white" p={5} borderRadius={"md"}>
            ...
            <Box borderRadius="md" my={4}>
              ...
              >
                {lesson?.description ? (
                  ...
                  <Center py={5}>
                    <Text fontSize={12} color={"gray.300"} fontWeight={500}>
                      Description Preview will be displayed here
                    </Text>
                  </Center>
                )}
              </Box>
              ...
              <ReactQuill
                ...
                style={{
                  ...
                  boxShadow:
                    "0 2px 4px 0 rgba(0, 0, 0, 0.11), 0 3px 10px 0 rgba(0, 0, 0, 0.1)",
                }}
                onChange={(e) =>
                  setLesson({
                    ...lesson,
            ...
            >
              {lesson?.download ? (
                ...
                <Box borderBottom="1px" borderColor="gray.50"></Box>
              )}
            </Box>
          </Box>
              ...
              <RadioGroup>
                <Stack direction="column">
                  ...
                  >
                    <Stack>
                      <Radio
                        // checked={lesson?.status === "draft"}
                        // defaultChecked={
                        //   lesson?.status === "draft" ? true : false
                        // }
                        value="draft"
                        onChange={(e) =>
                          handleRadioButton(e.target.checked, e.target.value)
                        }
                      >
                        Draft
                      </Radio>
                      <Radio
                        ...
                        // defaultChecked={
                        //   // lesson?.status === "published" ? true : false
                        //   true
                        // }
                        value="published"
                        onChange={(e) =>
